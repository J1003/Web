...
<%
	...
	p.setTotalPage();
	System.out.println("전체 게시글 수 : " + p.getTotalRecord()); //전체 게시물 출력 
	System.out.println("전체 페이지 수 : " + p.getTotalPage()); //전체 페이지 출력 
	
	/* 위에가 더 간단한 방법
	   System.out.println("total : " + BbsDAO.getTotalCount());
	   p.setTotalRecord(BbsDAO.getTotalCount());
	   System.out.println("p : " + p); //전체 게시물 출력 
	*/
	
	// 2. 현재 페이지 구하기
	String cPage = request.getParameter("cPage");
    if (cPage != null) { //null이면 Paging.java에 nowPage(현재페이지) 값을 쓴다.
	...
	// 4-1. 시작페이지 구하기
	int beginPage = 
		(p.getNowPage() - 1) / p.getPagePerBlock() * p.getPagePerBlock() + 1;
	p.setBeginPage(beginPage);
	p.setEndPage(beginPage + p.getPagePerBlock() - 1);
	
	// 4-2. 끝페이지(endPage)가 전체 페이지 수(totalPage)보다 크면
	// 끝페이지를 전체 페이지 수로 변경처리(설정)
	if (p.getEndPage() > p.getTotalPage()) { //endPage가 totalpage보다 크면 
		p.setEndPage(p.getTotalPage()); //endPage를 totalpage로 set하자!
	}
		...
		<tfoot>
			<tr>
				<td colspan="4">
					...
					<%-- [다음으로]에 대한 사용여부 처리 --%>
					<c:if test="${pvo.endPage >= pvo.totalPage }"> <!-- 크면 링크 비활성화 -->
						<li class="disable">다음으로</li>
					</c:if>
					<c:if test="${pvo.endPage < pvo.totalPage }"> <!-- 작으면 링크 활성화	 -->
						<li>
